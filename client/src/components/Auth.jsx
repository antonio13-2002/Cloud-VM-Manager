...
function TotpForm(props) {
  ...
  const handleCancel = () => {
    if (location.state && location.state.from === '/orders' && location.state.orderIdForAuth) {
        navigate('/orders', { 
            state: { 
                totpCancelled: true, 
                orderId: location.state.orderIdForAuth 
            } 
        });
    } else {
        // standard behaviour (e.g. dashboard)
        navigate('/');
    }
  };
    ...
    <Row>
      ...
      <Col xs={4}>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Button className='my-2 mx-2' variant='danger' onClick={handleCancel}>Cancel</Button>
        </Form>
      </Col>
      <Col xs={4}></Col>
    </Row>
...
function LogoutButton(props) {
  return (
    <Button 
      ...
      className="fw-bold fs-5 px-4 border-2"
